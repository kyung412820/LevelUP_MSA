...
@FeignClient(name = "entity-service", url = "http://localhost8080")
public interface EntityServiceClient {
    ...
    GameResponseDto findGameById(@PathVariable("gmaeId") Long gameId);

    @GetMapping("/v2/users/findAllUsers")
    List<UserResponseDto> findAllUsers(@RequestBody List<Long> userIds);

    @GetMapping("/v1/games/findAllGames")
