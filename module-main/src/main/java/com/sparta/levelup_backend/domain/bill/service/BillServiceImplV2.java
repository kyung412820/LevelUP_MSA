...
public class BillServiceImplV2 implements BillServiceV2 {
	...
	public void createBill(Long userId, Long orderId) {
		...
		UserResponseDto user = userServiceClient.findUserById(userId); // tutor 정보

		OrderEntity findOrder = orderRepository.findByIdOrElseThrow(orderId);

		BillEntity bill = BillEntity.builder()
			.tutorId(findOrder.getProduct().getUserId())
	...
	public BillResponseDto findBillByTutor(Long userId, Long billId) {
		...
		return new BillResponseDto(bill, userServiceClient.findUserById(bill.getTutorId()),
			userServiceClient.findUserById(bill.getStudentId()));
	}
	...
	private Page<BillResponseDto> findAllBillsByUsers(Pageable pageable, Page<BillEntity> users) {
		...
		Map<Long, UserResponseDto> allUsersEntity = userServiceClient.findAllUsers(allUsers).stream().collect(
